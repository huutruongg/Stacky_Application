...
const RecruiterController = {
  forgotPassword: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const { error } = RecruiterValidation.forgotPasswordSchema.validate(
        req.body
      );
      ...
      const url = `${process.env.URL_CLIENT}/recruiter/reset-password/${data._id}`;
  ...
  resetPassword: async (req: Request, res: Response): Promise<void> => {
    try {
      // Validate the request body
      const { error } = RecruiterValidation.resetPasswordSchema.validate(
        req.body
      );
      if (handleValidationError(error, res)) return;
