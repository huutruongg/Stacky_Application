...
export default class JobPostRepository extends BaseRepository<IJobPost> {
  ...
  public findJobPostByUserId = async (
    userId: string
  ...
  public getJobPostsByPage = async (
    page: number,
    pageSize: number
  ): Promise<IJobPost[]> => {
    return await this.model
      .find({ invisible: false })
      .skip(page * pageSize)
      .limit(pageSize)
      .exec();
  };
  ...
  public addApplicantToJobPost = async (
    jobPostId: string,
    newApplicant: IApplicant
  ): Promise<boolean> => {
  ...
  public getJobPostedByRecruiter = async (
    userId: string
  ): Promise<IJobPost[]> => {
    return await this.model
      .find({ userId: new Types.ObjectId(userId), invisible: false })
      .select(
        "_id userId jobTitle orgName typeOfIndustry candidatesLimit postedAt applicationDeadline jobImage jobSalary location"
      )
      .lean()
      .exec();
  };
