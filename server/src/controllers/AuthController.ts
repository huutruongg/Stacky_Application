...
export default class AuthController extends BaseController {
    ...
    private createGoogleStrategy() {
        return new GoogleStrategy(googleOAuthConfig, async (accessToken, _, profile, done) => {
            try {
                const email = profile.emails?.[0]?.value || '';
                const user = await this.authService.handleUserOAuth(
                    Provider.GOOGLE, profile.id, email, profile.displayName, accessToken
                );
    ...
    private createGitHubStrategy() {
        return new GitHubStrategy(githubOAuthConfig, async (accessToken, _, profile, done) => {
            try {
                const email = await this.getGitHubEmail(profile, accessToken);
                const user = await this.authService.handleUserOAuth(
                    Provider.GITHUB, profile.id, email, profile.displayName, accessToken
                );
                done(null, user || false);
            } catch (error: any) {
                // Handle the error and pass it to the next stage
    ...
    private async getGitHubEmail(profile: any, accessToken: string): Promise<string> {
        try {
            const response = await axios.get('https://api.github.com/user/emails', {
                headers: { Authorization: `token ${accessToken}` },
            });
            const primaryEmail = response.data.find((e: any) => e.primary)?.email;
            if (!primaryEmail) throw new Error('Cannot retrieve email from GitHub account');
            return primaryEmail;
        } catch (error) {
            log(error);
            return '';
                ...
                passport.authenticate(
                    ...
                    async (err: any, user: IUser, info: any) => {
                        if (err || !user) {
                            ...
                            res.status(401).json({ success: false, message: info?.message || "Authentication failed" });
                            return;
                        }
    ...
    public async register(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            return this.sendResponse(res, 200, { success: true, message: 'Registered successfully' });
        } catch (error) {
            next(error);
        }
    }
    ...
    public async regenerateAccessToken(req: Request, res: Response) {
        ...
        if (!refreshToken) return this.sendError(res, 401, 'Refresh token not found');
        ...
        try {
            ...
            const accessToken = await this.authService.generateAccessToken(String(user._id), user.privateEmail, user.role);
            if (!accessToken) return this.sendError(res, 401, 'Invalid refresh token');
